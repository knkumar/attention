...
class Images:
    ...
    def createGabor(self, size=(50,50)):
        ...
        self.gabor_horizontal = Enhance.Contrast(gabor_hor).enhance(2.7).resize(size, img.ANTIALIAS) 
        gabor_ver = img.fromarray( gabor(1000,"vertical",0.7)).convert("RGB")
        self.gabor_vertical = Enhance.Contrast(gabor_ver).enhance(2.7).resize(size, img.ANTIALIAS)
    ...
    def createDistract(self):
        self.createGabor()
        im_hor = Enhance.Contrast( rectangle(self.gabor_horizontal,"green") ).enhance(0.6)
        im_ver = Enhance.Contrast( rectangle(self.gabor_vertical,"green") ).enhance(0.6)
